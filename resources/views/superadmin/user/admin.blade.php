                ...
                <div class="card card-body card-fill">
                    <div class="clearfix">
                        <h5 class="font-weight-bold float-left">
                            {{__('words.admin.user.admin_list')}}
                        </h5>
                        <form action="" class="form-inline float-right" method="post">
                            @csrf
                            <input type="search" name="user_keyword" class="form-control form-control-sm mr-2" value="{{$admin_keyword}}" placeholder="{{__('words.admin.user.search')}}..." />
                            <button type="button" class="btn btn-sm btn-primary float-right mt-2 mt-md-0" id="btn-add-admin"><i class="fas fa-user-plus mr-1"></i>{{__('words.admin.user.add_new')}}</button>
                        </form>
                    </div>
                    <div class="table-responsive mt-2">
                        <table class="table table-bordered table-hover">
                            ...
                            <tr class="bg-blue">
                                ...
                                <th>{{__('words.admin.user.username')}}</th>
                                ...
                                <th>{{__('words.admin.user.role')}}</th>
                                <th style="width:150px">{{__('words.admin.user.action')}}</th>
                            </tr>
                            </thead>
                            <tbody>
                            @forelse ($data_admin as $item)
                                <tr>
                                    <td>{{ (($data_admin->currentPage() - 1 ) * $data_admin->perPage() ) + $loop->iteration }}</td>
                                    ...
                                    <td class="role">
                                        @foreach($role as $role_item)
                                            @if($role_item->admin_roles)
                                                @foreach($role_item->admin_roles as $role_admin)
                                                    @if($item->id == $role_admin->admin_id)
                                                        {{$role_item->title }}
                                                    @endif
                                                @endforeach
                                            @endif
                                    ...
                                    <td class="py-1">
                                        <a href="javascript:;" class="btn btn-sm btn-primary btn-icon mr-1 btn-edit-admin" data-id="{{$item->id}}" data-toggle="tooltip" title="{{__('words.admin.user.edit')}}"><i class="fa fa-edit"></i></a>
                                        <a href="{{route('superadmin.user.admin_delete', $item->id)}}" class="btn btn-sm btn-danger btn-icon mr-1 btn-confirm" data-toggle="tooltip" title="{{__('words.admin.users.delete')}}"><i class="fas fa-trash-alt"></i></a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                        ...
                        <div class="clearfix mt-2">
                            <div class="float-left" style="margin: 0;">
                                <p>Total <strong style="color: red">{{ $data_admin->total() }}</strong> Items</p>
                            </div>
                            <div class="float-right" style="margin: 0;">
                                {!! $data_admin->appends([
                                    'admin' => $data_admin->currentPage(),
    ...
    <div class="modal fade" id="addAdminModal">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="" id="create_admin_form" method="post">
                    @csrf
    ...
    <div class="modal fade" id="editAdminModal">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="" id="edit_admin_form" method="post">
                    @csrf
...
@section('script')
    <script>
        $(document).ready(function () {
            let role_str = "{{ json_encode($role) }}".replace(/\&quot;/g, '"');
            let total_roles = JSON.parse(role_str);
            total_roles.forEach(item => {
                let temp = '<option value=' + item.id +  '>' + item.title +  '</option>';
                $("#create_admin_form .role").append(temp);
            });
            $("#create_admin_form .role").select2();
            $("#btn-add-admin").click(function(){
                $("#create_admin_form input.form-control").val('');


                $("#addAdminModal").modal();
            });

            $("#create_admin_form .btn-submit").click(function(){
                $(".page-loader-wrapper").fadeIn();
                $.ajax({
                    url: "{{route('superadmin.user.admin_create')}}",
                    type: 'POST',
                    dataType: 'json',
                    ...
                    success : function(response) {
                        ...
                        if(response.status.msg == 'success') {
                            ...
                            if(messages.name) {
                                $('#create_admin_form .name_error strong').text(messages.name[0]);
                                $('#create_admin_form .name_error').show();
                                $('#create_admin_form .name').focus();
                            }

                            if(messages.username) {
                                $('#create_admin_form .username_error strong').text(messages.username[0]);
                                $('#create_admin_form .username_error').show();
                                $('#create_adin_form .username').focus();
                            }

                            if(messages.phone_number) {
                                $('#create_admin_form .phone_number_error strong').text(messages.phone_number[0]);
                                $('#create_admin_form .phone_number_error').show();
                                $('#create_admin_form .phone_number').focus();
                            }

                            if(messages.password) {
                                $('#create_admin_form .password_error strong').text(messages.password[0]);
                                $('#create_admin_form .password_error').show();
                                $('#create_admin_form .password').focus();
                            }
                        }
                    },
            ...
            $(".btn-edit-admin").click(function(){
                ...
                $("#edit_admin_form .id").val(id);
                $("#edit_admin_form .username").val(username);
                $("#edit_admin_form .email").val(email);
                $("#edit_admin_form .company").val(company);
                $("#edit_admin_form .phone_number").val(phone_number);
                $("#edit_admin_form .street").val(street);
                $("#edit_admin_form .house_number").val(house_number);
                $("#edit_admin_form .zip_code").val(zip_code);
                $("#edit_admin_form .city").val(city);
                $("#edit_admin_form .federal_state").val(federal_state);
                $("#edit_admin_form .country").val(country);
                $("#edit_admin_form .tax_id").val(tax_id);


                $("#editAdminModal").modal();
            });

            $("#edit_admin_form .btn-submit").click(function(){
                $(".page-loader-wrapper").fadeIn();
                $.ajax({
                    url: "{{route('superadmin.user.admin_edit')}}",
                    type: 'POST',
                    dataType: 'json',
                    ...
                    success : function(response) {
                        ...
                        if(response.status.msg == 'success') {
                            ...
                            if(messages.username) {
                                $('#edit_admin_form .username_error strong').text(messages.username[0]);
                                $('#edit_admin_form .username_error').show();
                                $('#edit_admin_form .username').focus();
                            }

                            if(messages.phone_number) {
                                $('#edit_admin_form .phone_number_error strong').text(messages.phone_number[0]);
                                $('#edit_admin_form .phone_number_error').show();
                                $('#edit_admin_form .phone_number').focus();
                            }

                            if(messages.password) {
                                $('#edit_admin_form .password_error strong').text(messages.password[0]);
                                $('#edit_admin_form .password_error').show();
                                $('#edit_admin_form .password').focus();
                            }
                        }
                    },
                    error: function(response) {
