                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            @forelse($data_voiceover_sample as $item)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('admin.service_lang.delete_voicesample_file', $item->id)}}" class="btn-confirm" style="color: red;">Delete</a>
                                    </td>
                                </tr>
                            @empty
    ...
    <script>
        $(document).ready(function () {
            $("#status_flag").change(function () {
                ...
                let self = $(this);
                $.ajax({
                    url: "{{route('admin.service_lang.status_flag', $lang[0]->id)}}",
                    type: "POST",
                    dataType: "json",
                    data: {flag: flag},
            ...
            var myDropzone = new Dropzone(".js-dropzone", {
                    url: '{{ route('admin.service_lang.upload_sample_file') }}',
                    headers: {
                    ...
                    complete: function (file, response) {
                        if (this.getUploadingFiles().length === 0 && this.getQueuedFiles().length === 0) {
                            ...
                            location.href = '/admin/service_lang/edit/{{ $lang[0]->id }}';
                        }
                    }
            });
